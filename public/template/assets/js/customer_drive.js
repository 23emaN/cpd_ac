 ...
 *
 * กติกาที่ยึดทั้งไฟล์
 *   1. เทียบ result แบบเข้ม (=== 1) จึงต้องแน่ใจว่าเซิร์ฟเวอร์ส่งเลขเสมอ
...
window.CustomerDrive = (function () {
     ...
     * โฟกัสกลับเข้า modal ทันที ทำให้กดหรือพิมพ์อะไรใน Swal ที่ลอยอยู่นอก modal ไม่ติดเลย
     */
    ...
    /** ตอบกลับมาตรฐาน {result, msg} — คืน true เมื่อสำเร็จ */
    function handled(res) {
        ...
        warn((res && res.msg) || 'ไม่ทราบสาเหตุ');
        return false;
    }
    ...
    function menuFor(node) {
        ...
        if (selected.length > 1) {
            ...
            items.push({ act: 'download', icon: 'ri-download-2-line', label: 'ดาวน์โหลดเป็น .zip' });

            if (cfg.perm.cdrive_edit) {
                items.push({ act: 'move', icon: 'ri-drag-move-2-line', label: 'ย้ายไป…' });
            }

            if (cfg.perm.cdrive_delete) {
                items.push({ sep: true });
                items.push({ act: 'delete', icon: 'ri-delete-bin-line', label: 'ทิ้งลงถังขยะ', danger: true });
            }

            items.push({ sep: true });
            items.push({ act: 'clear', icon: 'ri-close-circle-line', label: 'ยกเลิกการเลือก' });
        ...
        if (node.kind === 'folder') {
            if (node.kids) {
                items.push({
                    act: 'toggle',
                    icon: collapsed[node.id] ? 'ri-arrow-down-s-line' : 'ri-arrow-up-s-line',
                    label: collapsed[node.id] ? 'ขยายโฟลเดอร์' : 'ย่อโฟลเดอร์'
                });
            }

            items.push({ act: 'open', icon: 'ri-folder-open-line', label: 'เปิดเฉพาะโฟลเดอร์นี้' });
        } else if (node.preview) {
            items.push({ act: 'preview', icon: 'ri-eye-line', label: 'เปิดดู' });
        }

        items.push({ act: 'download', icon: 'ri-download-2-line', label: node.kind === 'folder' ? 'ดาวน์โหลดเป็น .zip' : 'ดาวน์โหลด' });

        if (node.kind === 'file' && node.version > 0) {
            items.push({ act: 'versions', icon: 'ri-history-line', label: 'ประวัติเวอร์ชัน' });
        }

        if (cfg.perm.cdrive_edit) {
            items.push({ sep: true });
            items.push({ act: 'rename', icon: 'ri-pencil-line', label: 'เปลี่ยนชื่อ' });
            items.push({ act: 'move', icon: 'ri-drag-move-2-line', label: 'ย้ายไป…' });
        }
    ...
    function showMenu(node, x, y) {
        ...
        menuFor(node).forEach(function (item) {
            ...
            out += '<button type="button" class="cd-menu-item' + (item.danger ? ' danger' : '') +
                '" data-act="' + item.act + '">' +
                '<i class="cd-ic fs-18 ' + item.icon + '"></i>' +
    ...
    function doAction(act, node) {
        ...
        if (selected.length > 1) {
            if (act === 'download') {
                window.location = fileUrl('download_zip', { node_ids: selected.join(',') });
                return;
            }
        ...
        switch (act) {
            ...
            case 'preview':
                window.open(fileUrl('download', { node_id: node.id, mode: 'inline' }), '_blank', 'noopener');
                break;

            case 'download':
                if (node.kind === 'folder') {
                    window.location = fileUrl('download_zip', { node_ids: node.id });
                } else {
                    window.location = fileUrl('download', { node_id: node.id });
                }
                break;
    ...
    function startRename(id) {
        ...
        function finish(save) {
            ...
            post('rename', { node_id: id, name: value }).done(function (res) {
                if (handled(res)) {
                    reload();
                } else {
                    $name.text(current);
    ...
    function confirmDelete(ids, label) {
        Swal.fire({
            ...
            post('delete', { node_ids: ids.join(',') }).done(function (res) {
                if (handled(res)) {
    ...
    function openVersions(id) {
        html('versions', { node_id: id }).done(function (markup) {
    ...
    function openPicker(ids) {
        html('folder_tree', { node_ids: ids.join(',') }).done(function (markup) {
    ...
    function openLinks() {
        html('link_list').done(function (markup) {
    ...
    function showLinkResult(res, isReset) {
        ...
        $box.removeClass('d-none').html(
            ...
            '<i class="cd-ic ri-checkbox-circle-line" aria-hidden="true"></i>' +
            '<div>' +
            ...
            '<i class="cd-ic ri-file-copy-line" aria-hidden="true"></i>' +
            ...
            '<i class="cd-ic ri-error-warning-line" aria-hidden="true"></i>' +
     ...
     *
     * วาง textarea ชั่วคราวไว้ใน modal เดียวกับปุ่มเสมอ (ไม่ใช่ document.body)
     * เพราะ Bootstrap ดักเหตุการณ์ focusin แล้วดึงโฟกัสกลับเข้า modal ทันที
     * ถ้าวางไว้นอก modal การเลือกข้อความจะหลุดก่อน execCommand จะทำงาน
     */
    ...
    function openTrash() {
        html('render_trash').done(function (markup) {
            fillModal('myModalLg', 'showModalLg', markup);
        });
    }

    function openActivity() {
        html('activity').done(function (markup) {
            fillModal('myModalLg', 'showModalLg', markup);
        });
    }
    ...
    function makeFolder() {
        Swal.fire({
            ...
            post('mkdir', { parent_id: uploadTarget() || 0, name: $.trim(r.value) }).done(function (res) {
    ...
    function send(file, targetId) {
        ...
        $.ajax({
            ...
            if (res && res.result === 1) {
                ...
                $row.find('.cd-up-note').text((res && res.msg) || 'ล้มเหลว');
            }
        }).fail(function (xhr) {
            $row.addClass('failed');
            $row.find('.cd-up-bar').remove();
    ...
    function move(ids, targetId) {
        post('move', { node_ids: ids.join(','), target_id: targetId || 0 }).done(function (res) {
            if (handled(res)) reload();
        }).fail(function (xhr) { boom(xhr.responseText || ''); });
    }
    ...
    function bind() {
        ...
        $(document).on('click.cdup', '.cd-uploads-toggle', function () {
            ...
            var collapsed = $('#cd-uploads').hasClass('collapsed');
            $(this).find('.cd-ic')
                .toggleClass('ri-arrow-up-s-line', collapsed)
                .toggleClass('ri-arrow-down-s-line', !collapsed);
        });
        ...
        $(document).on('click.cdmodal', '[data-cd="trash-restore"]', function () {
            ...
            post('restore_trash', { node_id: id }).done(function (res) {
                if (handled(res)) {
                    setTrashCount(res.trash_count);
                    openTrash();
                    reload();
        ...
        $(document).on('click.cdmodal', '[data-cd="trash-purge"]', function () {
            ...
            Swal.fire({
                ...
                post('trash_purge', { node_id: id }).done(function (res) {
                    if (handled(res)) {
                        setTrashCount(res.trash_count);
                        openTrash();
                    }
        ...
        $(document).on('click.cdmodal', '[data-cd="lk-save"]', function () {
            ...
            post('link_create', data).done(function (res) {
                if (res && res.result === 1) {
                    ...
                    warn((res && res.msg) || 'สร้างลิงก์ไม่สำเร็จ');
                }
            }).fail(function (xhr) {
                boom(xhr.responseText || '');
            }).always(function () {
        ...
        $(document).on('click.cdmodal', '[data-cd="lk-revoke"]', function () {
            ...
            Swal.fire({
                ...
                post('link_revoke', { link_id: id }).done(function (res) {
        ...
        $(document).on('click.cdmodal', '[data-cd="lk-reset"]', function () {
            ...
            Swal.fire({
                ...
                post('link_reset_password', { link_id: id, password: r.value }).done(function (res) {
                    if (res && res.result === 1) {
                        openLinks();
                        setTimeout(function () { showLinkResult(res, true); }, 400);
                    } else {
                        warn((res && res.msg) || 'ตั้งรหัสใหม่ไม่สำเร็จ');
                    }
                });
            });
        });
        ...
        $(document).on('click.cdmodal', '[data-cd="ver-download"]', function () {
            ...
            window.location = fileUrl('download', { node_id: nodeId, version: version });
        });
        ...
        $(document).on('click.cdmodal', '[data-cd="ver-restore"]', function () {
            ...
            Swal.fire({
                ...
                post('restore_version', { node_id: nodeId, version: version }).done(function (res) {
                    if (handled(res)) {
                        openVersions(nodeId);
                        reload();
                    }
