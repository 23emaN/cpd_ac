...
(function () {
    ...
    function render(res) {
        ...
        items.forEach(function (it) {
            html += '<div class="pt-item" data-id="' + it.id + '">' +
                ...
                (it.canDownload ? '<a class="pt-item-act" href="ajax/download.php?node_id=' + it.id + '">ดาวน์โหลด</a>' : '') +
    ...
    /** ยิงหนึ่งคำขอไปที่ upload_chunk.php แล้วคืน JSON */
    function chunkApi(form) {
        return fetch('ajax/upload_chunk.php', {
            method: 'POST',
            body: form,
            ...
            return chunkApi(status).then(function (res) {
                ...
                function step(tries) {
                    ...
                    return chunkApi(form).then(function (res) {
                        ...
                        return Promise.reject(new Error((res && res.msn) || 'ส่งไม่สำเร็จ'));
                    }).catch(function (e) {
                        if (tries > 1) {
    ...
    function finishRow(el, bar, note, res) {
        ...
        el.className = 'pt-up pt-up-fail';
        note.textContent = (res && res.msn) || 'ส่งไม่สำเร็จ';
    ...
    listEl.addEventListener('click', function (e) {
        ...
        Portal.post('ajax/remove_own.php', { node_id: id })
            .then(function (res) {
                if (res && res.expired) {
                    window.location.href = 'index.php';
                    return;
